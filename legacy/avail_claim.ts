...
const FROM =
  "0xa13347de937fB2Fc0D8CCa2a5fC95748f6F1fc64000000000000000000000000"; // SAFE PROXY ADDRESS as 32 bytes
...
async function main() {
  try {
    ...
    while (true) {
      ...
      console.log(
        `📦 Block inclusion number ${txSendBlockNumber}, head block number ${mappingResponse.blockNumber}`,
      );
            ...
            availApi.tx.vector
              ...
              .signAndSend(account, { nonce: -1 }, (result: any) => {
                if (result.isInBlock || result.isError) {
                  console.log(
                    `⚽️ TX included in block: blockhash: ${result.status.asInBlock.toString()}, txHash: ${result.txHash.toString()}, txIndex: ${result.txIndex}`,
                  );
                  resolve(result);
                }
              });
      ...
      console.log(
        `⏳ Waiting for bridge inclusion commitment. This can take a while...`,
      );
