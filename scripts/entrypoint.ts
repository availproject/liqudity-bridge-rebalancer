...
import { availAccount } from "../utils/client";
...
export async function entrypoint() {
  ...
  try {
    ...
    const AMOUNT_TO_BRIDGE_FORMATTED = new BigNumber(AMOUNT_TO_BRIDGE)
      .dividedBy(10 ** 18)!
      .toFixed(4);
    const poolBalances = await getTokenBalance(api, availAccount.address);

    const low = [];
    if (poolBalances.gasCheckBase.lt(GAS_THRESHOLD)) low.push("Base");
    if (poolBalances.gasCheckEthereum.lt(GAS_THRESHOLD)) low.push("Ethereum");

    if (low.length) {
      throw new Error(
        `Gas below threshold on: ${low.join(", ")}. Please top up the pool account.`,
      );
    }
  ...
  *Current Balances:*
  ...
  - BASE: ${poolBalances.humanFormatted.basePoolBalance} tokens`,
          type: "info",
        });
        bridgingResult = await BASE_TO_AVAIL(api, AMOUNT_TO_BRIDGE);
        break;
      case THRESHOLD.gt(poolBalances.basePoolBalance):
        await sendNotificationChannel({
